// src/background/ai-services/deepseek/state-controller.ts

import { browserAPI } from "../../utils/browser/browser-api";

/**
 * State Controller - Xử lý trạng thái của DeepSeek chat
 */
export class StateController {
  ...
  static async isGenerating(tabId: number): Promise<boolean> {
    try {
      ...
      console.error(
        "[StateController] ❌ Error checking generation state:",
        error
      );
      return false;
    }
  ...
  static async stopGeneration(tabId: number): Promise<boolean> {
    try {
      ...
      console.error("[StateController] ❌ Error stopping generation:", error);
      return false;
    }
  }
  ...
  static async getCurrentInput(tabId: number): Promise<string> {
    try {
      ...
      console.error("[StateController] ❌ Error getting current input:", error);
      return "";
    }
  }
  ...
  static async getLatestResponse(tabId: number): Promise<string | null> {
    try {
      ...
      console.error(
        "[StateController] ❌ Error getting latest response:",
        error
      );
      return null;
    }
