...
//   private static async getClipboardCapture(
//     tabId: number,
//     requestId: string
//   ): Promise<string | null> {
//     try {
...
//       console.log(
//         `[PromptController] 🔍 Searching for clipboard capture for tab ${tabId}, request ${requestId}`
//       );

//       const result = await new Promise<any>((resolve) => {
//         browserAPI.storage.session.get(null, (data: any) => {
//           resolve(data || {});
//         });
//       });

//       // Tìm tất cả clipboard captures
//       const captureKeys = Object.keys(result).filter((key) =>
//         key.startsWith("clipboardCapture_")
//       );

//       console.log(
//         `[PromptController] 📊 Found ${captureKeys.length} clipboard captures in storage`
//       );

//       if (captureKeys.length === 0) {
//         console.log(
//           "[PromptController] ⚠️ No clipboard captures found in storage"
//         );
//         return null;
//       }

//       // Tìm capture gần đây nhất (trong vòng 10 giây)
//       const now = Date.now();
...
//         const captureAge = now - capture.timestamp;

//         console.log(
//           `[PromptController] 📋 Capture ${key}: age=${captureAge}ms, source=${capture.source}, length=${capture.length}`
//         );

//         // Chỉ xem xét captures trong vòng 10 giây và từ auto-click
//         if (
//           captureAge < 10000 &&
//           (capture.source === "auto_click_dom_extraction" ||
//             capture.source === "auto_click_clipboard_fallback")
//         ) {
//           if (capture.timestamp > latestTimestamp) {
//             latestTimestamp = capture.timestamp;
...
//       if (latestCapture) {
//         console.log(
//           `[PromptController] ✅ Using clipboard capture from ${latestTimestamp}: ${latestCapture.source}, ${latestCapture.length} chars`
//         );

//         // 🆕 Clean up: xóa capture đã sử dụng để tránh duplicate
//         try {
//           await new Promise<void>((resolve) => {
//             browserAPI.storage.session.remove(
//               [`clipboardCapture_${latestTimestamp}`],
//               () => {
//                 console.log(
//                   `[PromptController] 🧹 Cleaned up used clipboard capture`
//                 );
//                 resolve();
//               }
//             );
...
//         } catch (cleanupError) {
//           console.warn(
//             "[PromptController] ⚠️ Failed to clean up clipboard capture:",
//             cleanupError
//           );
//         }

//         return latestCapture.text;
//       } else {
//         console.log(
//           "[PromptController] ⚠️ No recent valid clipboard capture found"
//         );

//         // Debug: log tất cả captures không hợp lệ
//         captureKeys.forEach((key) => {
//           const capture = result[key];
//           const captureAge = now - capture.timestamp;
//           console.log(
//             `[PromptController] 🐛 Invalid capture: ${key}, age=${captureAge}ms, source=${capture.source}`
//           );
//         });
//       }
...
//     } catch (error) {
//       console.error(
//         "[PromptController] ❌ Failed to get clipboard capture:",
//         error
//       );
//       console.error(
...
//           // 🆕 ENHANCED: Lấy response từ DOM (primary method)
//           let rawResponse = await this.getLatestResponseDirectly(tabId);

//           // 🆕 Thử click copy để capture qua watcher (secondary method)
//           const copyClicked = await this.clickCopyButtonAI(tabId);
//           console.log(
//             `[PromptController] 📋 Copy button clicked: ${copyClicked}`
//           );

//           // 🆕 Nếu DOM extraction failed, đợi thêm và thử đọc từ watcher storage
//           if (!rawResponse) {
//             console.log(
//               "[PromptController] ⚠️ DOM extraction failed, waiting for clipboard capture..."
//             );
//             await new Promise((resolve) => setTimeout(resolve, 1000));

//             // Thử lấy từ clipboard storage
//             const clipboardResponse = await this.getClipboardCapture(
//               tabId,
//               capturedRequestId
//             );
...
//               console.log(
//                 "[PromptController] ✅ Retrieved response from clipboard capture"
//               );
//             } else {
//               // Final fallback: thử DOM extraction lần cuối
//               console.log(
//                 "[PromptController] 🔄 Final attempt: retrying DOM extraction..."
//               );
//               await new Promise((resolve) => setTimeout(resolve, 500));
//               rawResponse = await this.getLatestResponseDirectly(tabId);
//             }
//           }
...
//   private static async clickCopyButtonAI(tabId: number): Promise<boolean> {
//     try {
//       // 🆕 Bước 1: Đánh dấu auto-click trong watcher
//       await executeScript(tabId, () => {
//         if (typeof (window as any).markAutoClick === "function") {
//           (window as any).markAutoClick();
//         }
//       });

//       // 🆕 Bước 2: Đợi một chút để watcher nhận marker
//       await new Promise((resolve) => setTimeout(resolve, 100));

//       // Bước 3: Click copy button như bình thường
//       const clickResult = await executeScript(tabId, () => {
//         const iconGroups = Array.from(
//           document.querySelectorAll(".ds-flex._965abe9._54866f7")
//         );

//         if (iconGroups.length === 0) {
//           return {
//             success: false,
//             reason: "no_icon_groups_found",
//           };
//         }

//         const lastGroup = iconGroups[iconGroups.length - 1];
//         const copyButton = lastGroup.querySelector(
...
//         // 🆕 Thử nhiều cách click để trigger clipboard
//         let clicked = false;
...
//           clicked = true;
//         } catch (e) {
//           console.error("Standard click failed:", e);
//         }

//         // Cách 2: Dispatch event
//         if (!clicked) {
//           try {
...
//       if (clickResult && clickResult.success) {
//         console.log(
//           `[PromptController] ✅ AI Copy button clicked successfully for tab ${tabId}`
//         );

//         // 🆕 Đợi lâu hơn cho watcher xử lý
//         await new Promise((resolve) => setTimeout(resolve, 500));
//         return true;
//       } else {
//         console.warn(
//           `[PromptController] ⚠️ Failed to click AI copy button:`,
//           clickResult
//         );
//         return false;
//       }
//     } catch (error) {
//       console.error(
//         `[PromptController] ❌ Exception while clicking AI copy button:`,
//         error
//       );
//       return false;
//     }
